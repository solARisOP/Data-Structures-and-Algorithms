...
int maximalAreaOfSubMatrixOfAll1(vector<vector<int>> &mat, int n, int m)
{
	...
	for(int i = 0; i<n; i++)
	{
		for(int j = 0; j<m; j++)
		{
            // below two steps are for making an histogram for every row 
			if(mat[i][j] == 1) h[j]++; // if its 1 we add 
			else h[j] = 0; // if its 0 we make it 0 otherwise it doesn't make any sense
		}
		int area = areaofhistogram(h);
		maxi = max(maxi, area);// for every row we make an histogram and compute its area and keep track of it
	}
